...
SELECT (SELECT TOP 1 MemMemberRegistrationId FROM MamAccountOpening  WHERE AccountNo = AcctNo AND UsmOfficeId=(SELECT USMOFFICEID FROM USMOFFICE WHERE OfficeName=@OFFICENAME))
	...
	FROM NexGenCoSysDBDev..MamAccountOpening where CAST(Remarks AS nvarchar) like '10402%' AND UsmOfficeId=(SELECT USMOFFICEID FROM USMOFFICE WHERE OfficeName=@OFFICENAME))
...
SELECT (SELECT TOP 1 MemMemberRegistrationId FROM MamAccountOpening  WHERE AccountNo = AcctNo AND UsmOfficeId=(SELECT USMOFFICEID FROM USMOFFICE WHERE OfficeName=@OFFICENAME))
      ...
      ,CAST(Typeoftranname AS NVARCHAR)
      ,TranNo
  FROM kamana..[transaction] WHERE AcctNo IN 
 (SELECT CAST(Remarks AS nvarchar) FROM NexGenCoSysDBDev..MamAccountOpening where CAST(Remarks AS nvarchar) like '10402%' AND UsmOfficeId=(SELECT USMOFFICEID FROM USMOFFICE WHERE OfficeName=@OFFICENAME)) 
