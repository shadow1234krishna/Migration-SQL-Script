...
SELECT (SELECT TOP 1 MemMemberRegistrationId FROM MamAccountOpening  WHERE AccountNo = AcctNo AND UsmOfficeId=(SELECT USMOFFICEID FROM USMOFFICE WHERE OfficeName=@OFFICENAME))
      ...
      ,TranNo
  FROM kamana..[transaction] WHERE AcctNo IN (SELECT AccountNo FROM MamAccountOpening WHERE AccountNo LIKE '10401%'
  OR AccountNo LIKE '10404%' AND UsmOfficeId=(SELECT USMOFFICEID FROM USMOFFICE WHERE OfficeName=@OFFICENAME))
