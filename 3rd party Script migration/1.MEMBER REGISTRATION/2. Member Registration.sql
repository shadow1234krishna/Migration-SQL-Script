...
DBCC CHECKIDENT ('MemMemberRegistration', RESEED, 0)
		...
		SELECT 
			  'MR-01-'+ CAST(CusCidNo AS NVARCHAR) --member id
			  , (SELECT TOP 1 SycMemberTypeId FROM SycMemberType) --sycmembertype
			  ,(SELECT TOP 1 UsmOfficeId FROM UsmOffice) --usmoffice
		  ...
		  FROM Finact.DBO.fincif
		  ORDER BY CusCidNo

		  
 UPDATE NexGenCoSysDBDev.dbo.SycMasterCode
 SET MasterCodeValue=( SELECT MAX(REPLACE(MemberId,'mr-01-','')) FROM dbo.MemMemberRegistration)
 WHERE SycMasterCodeId=1
 
update MemMemberRegistration set BirthOnBS= REPLACE(BirthOnBS,'.','/')
update MemMemberRegistration set BirthOnBS= REPLACE(BirthOnBS,'-','/')


update MemMemberRegistration set RegistrationOnBS= (select DateNepali from Finact.dbo.DateNumber where RegistrationOnBS=dateNumber)

update MemMemberRegistration set RegistrationOn= dbo.ConvertDateNepaliToEnglish(RegistrationOnBS)

---------------------------------------------------dummy updates-----------------------------------------------------------	 
update MemMemberRegistration set CitizenShipIssuedOn='1989-09-02',CitizenShipIssuedOnBs=dbo.ConvertDateEnglishToNepali('1989-09-02')
update MemMemberRegistration set [SycStateVDCId]=2
